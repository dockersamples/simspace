 ...
 *
 * Two seams keep this one function usable from more than the lab app:
 *
 *   fetchText(url)  — how a referenced file is read. Defaults to `fetch`, which
 *                     is what a browser host wants. A build-time host (Astro
 *                     reading a content collection off disk) passes its own
 *                     reader, so the same parse/resolve code produces the config
 *                     at build time and the instructions can be pre-rendered.
 *   parseSlides     — how a deck's chapter markdown is split into slides. The
 *                     deck lives in the lab app, not in this package, so it
 *                     injects the splitter; a host that doesn't render decks
 *                     passes nothing and gets a clear error rather than a
 *                     silently empty deck.
 */
export async function loadLabspace(labUrl, options = {}) {
  const fetchText = options.fetchText || fetchTextOverNetwork;
  const parseSlides = options.parseSlides || null;
  ...
  // Rendering a deck needs the slide splitter, which the host injects. Failing
  // loudly here beats returning a deck whose every chapter has zero slides —
  // that reads as "the content is broken" rather than "this host doesn't do
  // decks", and the difference matters when the host is someone else's site.
  if (kind === "slides" && !parseSlides) {
    throw new Error(
      "labspace.yaml declares `kind: slides`, but no `parseSlides` was provided to loadLabspace(). This host does not render slide decks.",
    );
  }
